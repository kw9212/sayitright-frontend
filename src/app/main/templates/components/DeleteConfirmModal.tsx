...
export default function DeleteConfirmModal({ templates, isOpen, onConfirm, onCancel }: Props) {
  return (
    <Dialog open={isOpen} onOpenChange={(open) => !open && onCancel()}>
      <DialogContent
        className="max-w-md max-h-[80vh] overflow-hidden 
          flex flex-col bg-zinc-900 text-white border-zinc-700"
      >
        <DialogHeader>
          ...
          <DialogDescription className="text-zinc-300">
            선택한 <strong className="text-white">{templates.length}개</strong>의 템플릿을
            삭제하시겠습니까?
          </DialogDescription>
        </DialogHeader>
        ...
        <div className="overflow-y-auto flex-1 space-y-4">
          ...
          >
            <p className="text-sm text-zinc-400 mb-2">삭제될 템플릿:</p>
            <ul className="space-y-2">
              {templates.map((template) => (
                <li key={template.id} className="flex items-start gap-2 text-sm">
                  <span className="text-red-400 mt-1">•</span>
                  <span className="flex-1 text-zinc-300 line-clamp-1">
                    {template.title || new Date(template.createdAt).toLocaleString('ko-KR')}
                  </span>
                </li>
              ))}
            </ul>
          ...
          <div className="bg-red-950/30 border border-red-700/50 rounded-lg p-4">
            ...
            <ul className="text-sm text-red-200 space-y-1">
              <li>
                • 삭제된 템플릿은 <strong>영구적으로 삭제</strong>됩니다.
              </li>
              <li>
